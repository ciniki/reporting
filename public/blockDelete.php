...
// Description
// -----------
// This method will delete an report blocks.
//
// Arguments
// ---------
// api_key:
...
// block_id:            The ID of the report blocks to be removed.
//
// Returns
// -------
//
function ciniki_reporting_blockDelete(&$ciniki) {
    //
    ...
    $rc = ciniki_core_prepareArgs($ciniki, 'no', array(
        ...
        'block_id'=>array('required'=>'yes', 'blank'=>'yes', 'name'=>'Report Blocks'),
        ));
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $rc = ciniki_reporting_checkAccess($ciniki, $args['tnid'], 'ciniki.reporting.blockDelete');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $strsql = "SELECT id, uuid, report_id, sequence "
        ...
        . "";
    $rc = ciniki_core_dbHashQuery($ciniki, $strsql, 'ciniki.reporting', 'block');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    if( !isset($rc['block']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.reporting.19', 'msg'=>'Report Blocks does not exist.'));
    }
    $block = $rc['block'];
    ...
    $rc = ciniki_core_objectCheckUsed($ciniki, $args['tnid'], 'ciniki.reporting.block', $args['block_id']);
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.reporting.20', 'msg'=>'Unable to check if the report blocks is still being used.', 'err'=>$rc['err']));
    }
    if( $rc['used'] != 'no' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.reporting.21', 'msg'=>'The report blocks is still in use. ' . $rc['msg']));
    }
