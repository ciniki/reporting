...
// -------
//
function ciniki_reporting_reportGet($ciniki) {
    //
    ...
    // check permission to run this function for this user
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'reporting', 'private', 'checkAccess');
    $rc = ciniki_reporting_checkAccess($ciniki, $args['tnid'], 'ciniki.reporting.reportGet');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    //
        ...
        $rc = ciniki_reporting_reportLoad($ciniki, $args['tnid'], $args['report_id']);
        if( $rc['stat'] != 'ok' ) {
            return $rc;
        }
